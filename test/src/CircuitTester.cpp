...
TEST(CircuitTester, DialaTest) {
    ...
    */
    ...
    for (bondIterator it = ms2.kindMap["PROTA"].bonds.cbegin(); it != ms2.kindMap["PROTA"].bonds.cend(); ++it){
    ...
    std::sort(cyclicCFAtoms.begin(), cyclicCFAtoms.end(), std::greater<std::vector<int>>());
    ...
    std::pair<cycleIterator, cycleIterator> itPair1(cyclicCFAtoms.cbegin(), cyclicFWAtoms.cbegin());
    for (; itPair1.first != cyclicCFAtoms.cend(); ++itPair1.first, ++itPair1.second){
        typedef std::vector<int>::const_iterator atomIterator;
        std::pair<atomIterator, atomIterator> itPair2((*itPair1.first).cbegin(), (*itPair1.second).cbegin());
        for (; itPair2.second != (*itPair1.second).cend(); ++itPair2.first, ++itPair2.second){
            ...
            if (*itPair2.first == *itPair2.second){
                ...
                std::cout << *itPair2.first << " not equal to " << *itPair2.second << std::endl;
            }
        }
    }
}
