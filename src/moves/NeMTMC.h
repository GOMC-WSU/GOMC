...
inline uint NEMTMC::GetBoxPairAndMol(const double subDraw, const double movPerc)
{
...
inline uint NEMTMC::Prep(const double subDraw, const double movPerc)
{
  ...
  if(state == mv::fail_state::NO_FAIL) {
    ...
    molInSourceBox = (double)molLookRef.NumKindInBoxSwappable(kindIndex, sourceBox);
...
inline uint NEMTMC::Transform()
{
  ...
  // index correspond to full interaction (lambda =1) and (lambdaWindow - X) is for destBox
  ...
  //We have not transferred the bonded energy. We apply it at the end. Note, We might
  //called regrowth, so we are concidering the newest conformaion.
...
inline void NEMTMC::CalcEnNEMT(uint lambdaIdxOldS, uint lambdaIdxNewS)
{
  ...
  recipDiffSource = calcEwald->ChangeLambdaRecip(oldMolNEMT.GetCoords(),
                                                lambdaOld_Coulomb_S,
                                                lambdaNew_Coulomb_S,
                                                molIndex, sourceBox);
  recipDiffDest = calcEwald->ChangeLambdaRecip(newMolNEMT.GetCoords(),
                                              lambdaOld_Coulomb_D,
                                              lambdaNew_Coulomb_D,
                                              molIndex, destBox);

  //need to contribute the self and correction energy
  W_recip = ((recipDiffSource + recipDiffDest +
...
void NEMTMC::AddWork()
{
  double W1 = ((newEnergy[sourceBox].Total() -
                           oldEnergy[sourceBox].Total()));
  double W2 = ((newEnergy[destBox].Total() -
