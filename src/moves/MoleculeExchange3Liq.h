...
// The acceptance criteria for GCMC simulation is similar to MEMC-3, but the
// GEMC simulation has different acceptance criteria
...
inline void MoleculeExchange3Liq::SetMEMC(StaticVals const &statV) {
  for (uint t = 0; t < exchangeRatioVec.size(); t++) {
    if (largeBBVec[t][0] != largeBBVec[t][1]) {
      printf("Error: In ME-3-Liq move, two atoms with same name should be used "
             "as backbone.\n");
  ...
  if (state == mv::fail_state::NO_FAIL) {
    ...
    if ((exchangeRatio == 1) ||
      ...
      if (state == mv::fail_state::NO_FAIL) {
        ...
        // We need to use the atomic coordinate of the seed atom (not the COM)
        // for regrowing the old molecule to generate correct Rosenbluth
        // weights.
...
inline uint MoleculeExchange3Liq::Prep(const double subDraw,
                                       const double movPerc) {
    ...
    for (uint n = 0; n < numMolInCavity[sourceBox]; n++) {
      ...
      if (n == 0) {
        ...
        newMol[sourceBox][n].SetSeed(cavityCenter[destBox], cavity, true, true,
                                     false);
        // Perform random rotational trial move around COM of small kind in
        // sourceBox
  ...
  if (ffRef.isFugacity) {
    ...
    if (sourceBox == mv::BOX0) {
      ...
      double ratioF =
          ...
          num::Factorial(totalMolInCavity[destBox] + exchangeRatio - 1);
      double ratioM = numTypeBDest / (numTypeADest + exchangeRatio);
      return exp(insA + delB) * ratioF * ratioM *
             pow(volCav, exchangeRatio - 1);
    }
  }
#endif
...
inline void MoleculeExchange3Liq::ShiftMol(uint box, const uint n,
...
inline void MoleculeExchange3Liq::RecoverMol(uint box, const uint n,
                                             const uint from, const uint to) {
