  ...
  IntraTargetedSwap(System &sys, StaticVals const& statV) :
    MoveBase(sys, statV), molLookRef(sys.molLookupRef), ffRef(statV.forcefield)
    {
      ...
      if(statV.intraTargetedSwapVal.enable) {
        for(int s = 0; s < (int) statV.intraTargetedSwapVal.targetedSwap.size(); s++) {
          config_setup::TargetSwapParam tsp = statV.intraTargetedSwapVal.targetedSwap[s];
          TSwapParam tempVar;
          ...
          for(int b = 0; b < tsp.selectedBox; b++) {
            for(int k = 0; k < (int) molRef.GetKindsCount(); k++) {
              ...
              ShiftAtomIndex += (int) thisKind.NumAtoms() * sys.molLookupRef.NumKindInBox(k, b);
            }
          }
          ...
          if (tsp.selectedResKind[0] == "ALL") {
            ...
            for (uint i = 0; i < tsp.selectedResKind.size(); i++) {
              ...
              if(!found) {
                printf("Error: In Intra-Targeted-Swap move, residue name %s cannot be moved or was not found in PDB/PSF file!\n",
                        kindName.c_str());
                exit(EXIT_FAILURE);
              }
            }
...
private:
  ...
  bool bulkMolIndexInSubVolume; // Picked molIndex in bulk is in subVolume or not
  //sub-Volume periodicity for each axis
    ...
    if(hasSubVolume[b]) {
      for(uint idx = 0; idx < targetSwapParam[b].size(); ++idx) {
        for(uint k = 0; k < targetSwapParam[b][idx].selectedResKind.size(); ++k) {
          ...
          char msg[257];
          sprintf(msg, "%% Accepted Intra-Targeted(%d)", targetSwapParam[b][idx].subVolumeIdx);
...
inline uint IntraTargetedSwap::Prep(const double subDraw, const double movPerc)
{
...
inline uint IntraTargetedSwap::GetBoxPairAndMol(const double subDraw, const double movPerc)
{
  ...
  if(!hasSubVolume[box]) {
    ...
    printf("Error: In Intra-Targeted-Swap move, no subVolume was defined for any Boxes!\n");
    exit(EXIT_FAILURE);
  }
  ...
  uint i = prng.randIntExc(targetSwapParam[box][pickedSubV].selectedResKind.size());
...
inline XYZ IntraTargetedSwap::GetSubVolumeCenter(const uint &box, const uint &subVIdx)
{
  int i;
  ...
  for(i = 1; i < listSize; ++i) {
    if(molLookRef.IsAtomInBox(tsp.atomList[i], box)) {
      ...
      printf("Error: In Intra-Targeted-Swap move, atom index %d does not exist in box %d!\n",
            tsp.atomList[i], box);
...
inline void IntraTargetedSwap::CheckSubVolumePBC(const uint &box)
{
  ...
  if(subVolDim.x > boxAxis.x) {
    printf("Error: In Intra-Targeted-Swap move, box dimension in X axis shrunk bellow %g A for Subvolume %d!\n",
           subVolDim.x, subVolIdx);
    shrunk = true;
  }
  if(subVolDim.y > boxAxis.y) {
    printf("Error: In Intra-Targeted-Swap move, box dimension in Y axis shrunk bellow %g A for Subvolume %d!\n",
           subVolDim.y, subVolIdx);
    shrunk = true;
  }
  if(subVolDim.z > boxAxis.z) {
    printf("Error: In Intra-Targeted-Swap move, box dimension in Z axis shrunk bellow %g A for Subvolume %d!\n",
           subVolDim.z, subVolIdx);
    shrunk = true;
  }

  // Wrap the center of subVolume and check for PBC
  subVolCenter = boxDimRef.WrapPBC(subVolCenter, box, xyzPBC[0], xyzPBC[1], xyzPBC[2]);
  ...
  if((unSlanCenter.x >= boxAxis.x) || (unSlanCenter.x < 0)) {
    printf("Error: In Intra-Targeted-Swap move, subVolume center.x %g A for Subvolume %d is out of range of axis.x %g A!\n",
  ...
  if((unSlanCenter.y >= boxAxis.y) || (unSlanCenter.y < 0)) {
    printf("Error: In Intra-Targeted-Swap move, subVolume center.y %g A for Subvolume %d is out of range of axis.y %g A!\n",
  ...
  if((unSlanCenter.z >= boxAxis.z) || (unSlanCenter.z < 0)) {
    printf("Error: In Intra-Targeted-Swap move, subVolume center.z %g A for Subvolume %d is out of range of axis.z %g A!\n",
  ...
  if(!xyzPBC[0]) { // no PBC in x axis
    ...
    if((unSlantMaxDim.x >= boxAxis.x) || (unSlantMinDim.x < 0)) {
      printf("Error: In Intra-Targeted-Swap move with no PBC in x axis, subVolume.x range [%g-%g] A for Subvolume %d is on the edge or out of range of axis.x %g A!\n",
  ...
  if(!xyzPBC[1]) { // no PBC in y axis
    ...
    if((unSlantMaxDim.y >= boxAxis.y) || (unSlantMinDim.y < 0)) {
      printf("Error: In Intra-Targeted-Swap move with no PBC in y axis, subVolume.y range [%g-%g] A for Subvolume %d is on the edge or out of range of axis.y %g A!\n",
  ...
  if(!xyzPBC[2]) { // no PBC in z axis
    ...
    if((unSlantMaxDim.z >= boxAxis.z) || (unSlantMinDim.z < 0)) {
      printf("Error: In Intra-Targeted-Swap move with no PBC in z axis, subVolume.z range [%g-%g] A for Subvolume %d is on the edge or out of range of axis.z %g A!\n",
...
inline uint IntraTargetedSwap::PickMolInSubVolume()
{
  ...
  // Need the number of molecule of the selected kind in subVolume for acceptance
  ...
  if(bulkToSubvolume) { // transfer from bulk to subvolume
    ...
    bulkMolIndexInSubVolume = (std::find(molIdxInSubVolume.begin(), molIdxInSubVolume.end(),
...
inline double IntraTargetedSwap::GetCoeff() const
{
  ...
  if(bulkToSubvolume) { // transfer from bulk to subVolume
    if(bulkMolIndexInSubVolume) {
      ...
      return (numInBulk / (numInSubVolume + 1.0)) * (subVolume * boxDimRef.volInv[box]);
    }
  } else { // transfer from subVolume to bulk
...
inline void IntraTargetedSwap::Accept(const uint rejectState, const ulong step)
{
...
uint IntraTargetedSwap::GetGrowingAtomIndex(const uint k)
{
  ...
  // https://codeforces.com/blog/entry/17974
  {
    ...
    if(centric == -1) {
      printf("Error: In Intra-Targeted-Swap move, no center atom was found for %s!\n",
              kind.name.c_str());
      exit(EXIT_FAILURE);
    }
                                      ...
                                      const bool useGC)
{
  ...
  if(useGC) { // uses the geometric center to detect the molecule in cavity
    switch (pbcMode) {
      case PBC_X:
        found = SearchCavity_GC<true, false, false>(molIdxInSubVolume,
                  subVolCenter, subVolDim, box, kind);
        break;
      case PBC_Y:
        found = SearchCavity_GC<false, true, false>(molIdxInSubVolume,
                  subVolCenter, subVolDim, box, kind);
        break;
      case PBC_Z:
        found = SearchCavity_GC<false, false, true>(molIdxInSubVolume,
                  subVolCenter, subVolDim, box, kind);
        break;
      case PBC_XY:
        found = SearchCavity_GC<true, true, false>(molIdxInSubVolume,
                  subVolCenter, subVolDim, box, kind);
        break;
      case PBC_XZ:
        found = SearchCavity_GC<true, false, true>(molIdxInSubVolume,
                  subVolCenter, subVolDim, box, kind);
        break;
      case PBC_YZ:
        found = SearchCavity_GC<false, true, true>(molIdxInSubVolume,
                  subVolCenter, subVolDim, box, kind);
        break;
      case PBC_XYZ:
        found = SearchCavity_GC<true, true, true>(molIdxInSubVolume,
                  subVolCenter, subVolDim, box, kind);
        break;
    ...
    switch (pbcMode) {
      case PBC_X:
        found = SearchCavity_AC<true, false, false>(molIdxInSubVolume,
                  subVolCenter, subVolDim, box, kind, growingAtomIndex[kind]);
        break;
      case PBC_Y:
      ...
      case PBC_YZ:
        ...
        break;
      case PBC_XYZ:
        found = SearchCavity_AC<true, true, true>(molIdxInSubVolume,
                  subVolCenter, subVolDim, box, kind, growingAtomIndex[kind]);
        break;
...
template <bool pbcX, bool pbcY, bool pbcZ>
bool IntraTargetedSwap::SearchCavity_AC(std::vector<uint> &mol, const XYZ& center,
                    const XYZ& cavDim, const uint box, const uint kind,
                    const int atomIdx)
{
  int *particleMol = molLookRef.molIndex;
