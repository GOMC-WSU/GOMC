...
inline void MoleculeExchange2Liq::SetMEMC(StaticVals const& statV)
{
  for(uint t = 0; t < exchangeRatioVec.size(); t++) {
    ...
    for(uint i = 0; i < 2; i++) {
      if(smallBB[i] == -1) {
        printf("Error: In ME-2-Liq move, atom name %s or %s was not found in %s residue.\n",
    ...
    if(molRef.kinds[kindSVec[t]].NumAtoms() > 1) {
      if(smallBB[0] == smallBB[1]) {
        printf("Error: In ME-2-Liq move, atom names in small molecule backbone cannot be same!\n");
        exit(EXIT_FAILURE);
      }
    }
...
inline uint MoleculeExchange2Liq::Prep(const double subDraw, const double movPerc)
{
  ...
  if(state == mv::fail_state::NO_FAIL) {
    ...
    for(uint n = 0; n < numMolInCavity[destBox]; n++) {
      ...
      // Set the Backbone of large molecule to perform rotational trial around it
    ...
    for(uint n = 0; n < numMolInCavity[sourceBox]; n++) {
      ...
      if(n == 0) {
        ...
        newMol[sourceBox][n].SetSeed(cavityCenter[destBox], cavity, true, true, true);
...
inline uint MoleculeExchange2Liq::Transform()
{
  ...
  for(uint n = 0; n < numMolInCavity[destBox]; n++) {
    ...
    molRef.kinds[kindIndex[destBox][n]].BuildIDOld(oldMol[destBox][n], molIndex[destBox][n]);
    //Add bonded energy because we don't consider it in DCRotate.cpp
  ...
  for(uint n = 0; n < numMolInCavity[destBox]; n++) {
    molRef.kinds[kindIndex[destBox][n]].BuildIDNew(newMol[destBox][n], molIndex[destBox][n]);
...
inline void MoleculeExchange2Liq::ShiftMol(uint box, const uint n, const uint from, const uint to)
{
...
inline void MoleculeExchange2Liq::RecoverMol(uint box, const uint n,const uint from, const uint to)
{
...
inline void MoleculeExchange2Liq::Accept(const uint rejectState, const ulong step)
{
