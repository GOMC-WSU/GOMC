...
DCFreeCycleSeed::DCFreeCycleSeed(DCData *data, const mol_setup::MolKind &kind,
                                 ...
                                 uint prev)
    : data(data), hed(data, kind, cycAtoms, focus, prev), anchorBond(0.0),
      anchorBondOld(0.0), bondEnergy(0.0), bondLength{{}}, bondLengthOld{{}},
      bondKinds{{}}, bondedInRing{{}} {
    : data(data), hed(data, kind, cycAtoms, focus, prev) {
      ...
      if (data->nLJTrialsNth < 1) {
        std::cout
            << "Error: CBMC secondary atom trials must be greater than 0.\n";
        exit(EXIT_FAILURE);
      }
    }
    ...
    void DCFreeCycleSeed::SetBondLengthNew(TrialMol & newMol) {
      for (uint i = 0; i < hed.NumBond(); ++i) {
        ...
        if (bondedInRing[i]) {
          bondLength[i] = newMol.GetBCoords()
                              .Difference(hed.Bonded(i), hed.Focus())
                              .Length();
        } else {
    ...
    void DCFreeCycleSeed::BuildNew(TrialMol & newMol, uint molIndex) {
      ...
      calc.ParticleInter(inter, real, positions[hed.NumBond()], overlap,
                         hed.Prev(), molIndex, newMol.GetBox(), nLJTrials);
      ...
      newMol.AddEnergy(Energy(hed.GetEnergy() + bondEnergy,
                              hed.GetNonBondedEn(), inter[winner], real[winner],
                              0.0, 0.0, 0.0));
    ...
    void DCFreeCycleSeed::BuildOld(TrialMol & oldMol, uint molIndex) {
      ...
      data->axes.UnwrapPBC(positions[hed.NumBond()], 0, 1, oldMol.GetBox(),
                           center);
      ...
      double stepWeight = 0;
      calc.ParticleInter(inter, real, positions[hed.NumBond()], overlap,
                         hed.Prev(), molIndex, oldMol.GetBox(), nLJTrials);
      ...
      oldMol.AddEnergy(Energy(hed.GetEnergy() + bondEnergy,
                              hed.GetNonBondedEn(), inter[0], real[0], 0.0, 0.0,
                              0.0));
