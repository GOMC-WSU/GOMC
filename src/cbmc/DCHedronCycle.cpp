                                 ...
                                 uint kind, uint nTrials, uint bType)
{
  ...
  if(angleInRing[bType][bType] || angleFix) {
    ...
    if(angleInRing[bType][bType]) {
      th = CalcTheta(newMol, bonded[bType], focus, prev);
    } else {
      th = data->ff.angles->Angle(kind);
    }
                                 ...
                                 uint kind, uint nTrials, uint bType)
{
  ...
  if(angleInRing[bType][bType] || angleFix) {
    double th;
    if(angleInRing[bType][bType]) {
      th = CalcTheta(oldMol, bonded[bType], focus, prev);
    } else {
      th = data->ff.angles->Angle(kind);
    }
    std::fill_n(data->angles, nTrials, th);
  ...
  for (uint b = 1; b < nBonds; ++b) {
    ...
    for (uint c = 0; c < b; ++c) {
      ...
      if(angleInRing[b][c] || angleFix) {
        ...
        double bfcRing;
        if(angleInRing[b][c]) {
          bfcRing = CalcTheta(newMol, bonded[b], focus, bonded[c]);
        } else {
          bfcRing = data->ff.angles->Angle(angleKinds[b][c]);
        }
  ...
  for (uint b = 1; b < nBonds; ++b) {
    ...
    for (uint c = 0; c < b; ++c) {
      ...
      if(angleInRing[b][c] || angleFix) {
        ...
        double bfcRing;
        if(angleInRing[b][c]) {
          bfcRing = CalcTheta(oldMol, bonded[b], focus, bonded[c]);
        } else {
          bfcRing = data->ff.angles->Angle(angleKinds[b][c]);
        }
        double var = (cos(bfcRing) - cosTerm) / sinTerm;
