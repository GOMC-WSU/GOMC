...
/* After the first run, the molecules are sorted, so we need to use the same sorting process
   ...
   After the first restart from checkpoint, the molecules have been sorted in the output
   So we use GetSortedMolNum 
   
   This indexing allows linear time sorting of the molecules.  It took much playing with,
   so modify with extreme care.
*/
