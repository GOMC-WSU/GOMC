                                    ...
                                    const uint box) const
{
  ...
  if (box < BOXES_WITH_U_NB) {
    ...
    for (uint p = 0; p < length; ++p) {
      uint atom = start + p;
      molCoordsToCell.push_back( cellList.PositionToCellNonInline(molCoords[atom], box ));
      std::cout << "pos2cell: " << cellList.PositionToCellNonInline(molCoords[atom], box ) << std::endl;

      if(0>cellList.PositionToCellNonInline(molCoords[atom], box )){
    ...
    for (uint p = 0; p < length; ++p) {
      ...
      for(int i = 0; i < (int) nIndex.size(); i++) {
        ...
        currentCoords[atom].x,currentCoords[atom].y,currentCoords[atom].z,nIndex[i],
        currentCoords[nIndex[i]].x,currentCoords[nIndex[i]].y,currentCoords[nIndex[i]].z
        );
        if (currentAxes.InRcut(distSq, virComponents, currentCoords, atom,
                               nIndex[i], box)) {
          printf("Dist %d %d %f\n", atom, nIndex[i], distSq);
