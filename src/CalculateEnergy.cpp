...
#else
#ifdef _OPENMP
  #pragma omp parallel for default(none) shared(cellStartIndex, cellVector, mapParticleToCell, neighborList) \
...
#ifdef _OPENMP
  #pragma omp parallel for default(none) shared(atom, nIndex) reduction(+:tempREn, tempLJEn)
#endif
