...
void ConfigSetup::verifyInputs(void)
{
  ...
  if(abs(sys.moves.displace + sys.moves.rotate + sys.moves.transfer +
         ...
         sys.moves.regrowth + sys.moves.multiParticle - 1.0) > 0.01) {
  ...
  if(abs(sys.moves.displace + sys.moves.rotate + sys.moves.intraSwap +
         sys.moves.volume + sys.moves.regrowth + sys.moves.multiParticle
         - 1.0) > 0.01) {
    std::cout << "Error: Sum of move frequncies are not equal to one!\n";
    exit(EXIT_FAILURE);
  }
  ...
  if(abs(sys.moves.displace + sys.moves.multiParticle + sys.moves.rotate + 
         sys.moves.intraSwap + sys.moves.transfer + 
...
#else
  if(abs(sys.moves.displace + sys.moves.multiParticle + sys.moves.rotate + 
         sys.moves.intraSwap + sys.moves.regrowth - 1.0) > 0.01) {
    std::cout << "Error: Sum of move frequncies are not equal to one!!\n";
    exit(EXIT_FAILURE);
  }
#endif
