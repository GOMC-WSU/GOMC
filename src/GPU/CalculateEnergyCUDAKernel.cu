...
void CallMolInterGPU(VariablesCUDA *vars,
  ...
  cudaMemcpy(&vars->gpu_x[atomNumber], newCoords.x, atomNumber * sizeof(double), cudaMemcpyHostToDevice);
  cudaMemcpy(&vars->gpu_y[atomNumber], newCoords.y, atomNumber * sizeof(double), cudaMemcpyHostToDevice);
  cudaMemcpy(&vars->gpu_z[atomNumber], newCoords.z, atomNumber * sizeof(double), cudaMemcpyHostToDevice);
    ...
    double gpu_rOn)
{
  ...
  double Cn = 1.0 / pow(gpu_rCut, pn) - An / 3.0 * (gpu_rCut - gpu_rOn) *
              (gpu_rCut - gpu_rOn) * (gpu_rCut - gpu_rOn) - Bn / 4.0 *
              (gpu_rCut - gpu_rOn) * (gpu_rCut - gpu_rOn) *
              (gpu_rCut - gpu_rOn) * (gpu_rCut - gpu_rOn);
  ...
  double B6 = -6.0 * (7.0 * gpu_rOn - 9.0 * gpu_rCut) /
              (pow(gpu_rCut, 8.0) * (gpu_rCut - gpu_rOn) * (gpu_rCut - gpu_rOn) *
              (gpu_rCut - gpu_rOn));
  double C6 = pow(gpu_rCut, -6.0) - A6 / 3.0 * (gpu_rCut - gpu_rOn) *
              (gpu_rCut - gpu_rOn) * (gpu_rCut - gpu_rOn) - B6 / 4.0 *
              (gpu_rCut - gpu_rOn) * (gpu_rCut - gpu_rOn) * (gpu_rCut - gpu_rOn) *
              (gpu_rCut - gpu_rOn);
