...
namespace {
// Assigns numerical molecule kind indices to all molKinds
...
int ReadPSF(const char *psfFilename, const uint box, MoleculeVariables &molVars,
  ...
  std::cout << "ReadPSFAtoms: " << std::chrono::duration<double, std::milli>(t2-t1).count() << "ms\n";
  ...
  */
  ...
  std::cout << "BondAdjacencyList: " << std::chrono::duration<double, std::milli>(t4-t3).count() << "ms\n";
  ...
  */
  ...
  std::cout << "createKindMap: " << std::chrono::duration<double, std::milli>(t6-t5).count() << "ms\n";
  ...
  std::cout << "firstAtomLookup: " << std::chrono::duration<double, std::milli>(t8-t7).count() << "ms\n";
  // find bond header+count
  // make sure molecule has bonds, appears before !NBOND
...
int ReadPSFExplicitNonbondExclusions(
  ...
  for (uint n = 0; n < nNonbondExclusions; n++) {
    int num = fscanf(psf, "%u %u %u %u", &imp.a0, &imp.a1, &imp.a2, &imp.a3);
    if (num != 4) {
      fprintf(stderr, "ERROR: Incorrect number of NNBs in PSF file ");
      return errors::READ_ERROR;
    } else if (feof(psf) || ferror(psf)) {
      fprintf(stderr, "ERROR: Could not find all NNBs in PSF file ");
      return errors::READ_ERROR;
    }
