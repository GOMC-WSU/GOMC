...
void DCDPlugin::print_dcderror(const char *func, int errcode) {
  const char *errstr;
 ...
 */
int DCDPlugin::read_dcdheader(fio_fd fd, int *N, int *NSET, int *ISTART, 
...
int DCDPlugin::read_charmm_extrablock(fio_fd fd, int charmm, int reverseEndian,
                                  float *unitcell) {
...
int DCDPlugin::read_fixed_atoms(fio_fd fd, int N, int num_free, const int *indexes,
...
int DCDPlugin::read_charmm_4dim(fio_fd fd, int charmm, int reverseEndian) {
 ...
 */
int DCDPlugin::read_dcdstep(fio_fd fd, int N, float *X, float *Y, float *Z, 
 ...
 *               next timestep.
 */
int DCDPlugin::skip_dcdstep(fio_fd fd, int natoms, int nfixed, int charmm) {
  long seekoffset = 0;
  long rec_scale;
 ...
 */
int DCDPlugin::write_dcdstep(fio_fd fd, int curframe, int curstep, int N, 
 ...
 */
int DCDPlugin::write_dcdheader(fio_fd fd, const char *remarks, int N, 
 ...
 */
void DCDPlugin::close_dcd_read(int *indexes, float *fixedcoords) {
  free(indexes);
  free(fixedcoords);
}
...
void * DCDPlugin::open_dcd_read(const char *path, const char *filetype, 
    int *natoms) {
  dcdhandle *dcd;
  fio_fd fd;
  int rc;
   ...
   */
  {
    ...
    if (dcd->nsets > 0 && newnsets != dcd->nsets) {
      printf("dcdplugin) Warning: DCD header claims %d frames, but \n"
             "dcdplugin) file size (%lu) indicates there are actually \n"
             "%d frames of size (%lu)\n", 
...
int DCDPlugin::read_next_timestep(void *v, int natoms, molfile_timestep_t *ts) {
  dcdhandle *dcd;
  int i, j, rc;
  float unitcell[6];
...
void DCDPlugin::close_file_read(void *v) {
...
void * DCDPlugin::open_dcd_write(const char *path, const char *filetype, 
    int natoms) {
  dcdhandle *dcd;
  fio_fd fd;
  int rc;
...
int DCDPlugin::write_timestep(void *v, const molfile_timestep_t *ts) { 
  dcdhandle *dcd = (dcdhandle *)v;
  int i, rc, curstep;
  float *pos = ts->coords;
  double unitcell[6];
...
void DCDPlugin::close_file_write(void *v) {
  dcdhandle *dcd = (dcdhandle *)v;
  fio_fclose(dcd->fd);
  free(dcd->x);
  free(dcd->y);
